...
use crate::stacked::vanilla::{
    challenges::SynthPoRep, Challenges, Column, ColumnProof, EncodingProof, LabelingProof,
...
impl<T: Domain, S: Domain> PublicInputs<T, S> {
    ...
    pub fn challenges(
        ...
        let k = k.unwrap_or(0);
        match challenges {
            Challenges::Orig(orig_challenges) => {
                let seed = self
                    .seed
                    .expect("challenge seed must be set for original porep");
                orig_challenges.derive(sector_nodes, &self.replica_id, &seed, k as u8)
            }
            Challenges::Synth(synth_challenges) => {
                ...
                match self.seed.as_ref() {
                    Some(seed) => {
                        // TODO vmx 2023-10-16: It may be possible to call the underlying function
                        // directly
                        synth_challenges.derive(
                            sector_nodes,
                            &self.replica_id,
                            &comm_r,
                            seed,
                    ...
                    None => {
                        ...
                        SynthPoRep::derive_synthetic(sector_nodes, &self.replica_id, &comm_r)
                    }
                }
            }
...
mod tests {
    ...
    fn test_public_params_identifier() {
        ...
        let setup_params_32gib = SetupParams {
            ...
            porep_id: [1u8; 32],
            challenges: Challenges::new_original(18),
            num_layers: 11,
