...
/// each challenge `c`. Historically there was a list of 6 possible `h` values, ranging from 7 to
/// 12. For production sector sizes, we always only use `h = 10`. It was the element at index 3
/// (the 4th element), hence the circuit takes `h_select = 2^3` as public input.
...
/// setup.
pub const fn circuit_hs(sector_nodes: usize) -> [usize; 6] {
    if sector_nodes <= SECTOR_SIZE_32_KIB {
        [1; 6]
    } else {
        [7, 8, 9, 10, 11, 12]
...
pub fn h_select(sector_nodes: usize, h: usize) -> u64 {
    ...
    let h_index = circuit_hs(sector_nodes)
        .iter()
